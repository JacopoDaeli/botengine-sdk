...
var _DialogAction = require('./DialogAction');
    ...
    value: function on(intent, dialogId, dialogArgs) {
      if (!this.handlers.hasOwnProperty(intent)) {
        if (Array.isArray(dialogId)) {
          this.handlers[intent] = _DialogAction2.default.waterfall(dialogId);
        } else if (typeof dialogId === 'string') {
          this.handlers[intent] = _DialogAction2.default.DialogAction.beginDialog(dialogId, dialogArgs);
        } else {
          this.handlers[intent] = _DialogAction2.default.waterfall([dialogId]);
        }
      } else {
