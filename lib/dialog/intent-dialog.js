...
var IntentDialog = function (_Dialog) {
  ...
  _createClass(IntentDialog, [{
      ...
      this.recognizeIntents(msg.language, msg.text, function (err, intents, entities) {
        if (!err) {
          ...
          session.compareConfidence(msg.language, msg.text, score, function (handled) {
            if (!handled) _this3.invokeIntent(session, intents, entities);
          });
        } else {
          session.endDialog({
              ...
              _this4.captureIntent({
                ...
                endDialog: function endDialog() {
                  action.endDialog({
                    ...
                    captured: { intents: intents, entities: entities }
                  });
                },
                send: action.send
              }, topIntent, entities);
    ...
    value: function getDefaultGroup() {
      ...
      if (!group) {
        group = new _intentGroup2.default(_constants2.default.Id.DefaultGroup);
        this.groups[_constants2.default.Id.DefaultGroup] = group;
      }
      return group;
    }
  }]);
